...
int sendrequest(int sd) 
{
  ...
  if (servhost && servport) {
	...
	//get HTTP server socket by connecttoserver with extracted servhost and servport
	int servsd = connecttoserver(servhost, atoi(servport));
	...
	//For testing purpose
	printf("The message write to the HTTP server is %s\n", msg);
	//write the HTTP request from client to HTTP server
	write(servsd, msg, MAXMSGLEN); 
...
char *readresponse(int sd)
{
	...
	//read the message from HTTP server socket
	int err = read(sd, msg, RESPMSGLEN-1);
	//if reading fails
	if (err == -1){
		perror("read");
		return(0);
